...
6.  Once signed in, select **Data** from the left-hand menu. Next, select **default** under Databases (if this does not appear, start your cluster). Finally, select **+** next to the Tables header.
...
2.  Within Azure Databricks, select **Workspace** on the left-hand menu, then **Users**, select your user, then select the down arrow on the top of your user workspace. Select **Import**.
...
5.  Before you begin, make sure you attach your cluster to the notebook, using the dropdown on the upper-left. You will need to do this for each notebook you open.

    ![Select your cluster to attach it to the notebook](media/attach-cluster-to-notebook.png)

6.  Run each cell of the notebook individually by clicking within the cell, then entering **Ctrl+Shift** on your keyboard. Pay close attention to the instructions within the notebook so you understand each step of the data preparation process.
...
1.  Open a new Command Prompt from the Azure Machine Learning Workbench, as outlined in Task 1111.
...
1.  On the New Lined Service (Azure Blob Storage) account screen, enter the following and then click **Finish**.
...
1.  Review settings on the **Summary** tab, but **DO NOT click Next**.
...
1.  Drag the Notebook activity onto the design surface to the right of the Copy activity.
...
1.  In Azure Databricks, select the Account icon in the top right corner of the window, then select **User Settings**.
...
1.  Leave the form open and switch back to Azure Databricks. Select **Clusters** on the left-hand menu, then select your cluster in the list. Select the **Tags** tab and copy the **ClusterId** value.
...
1.  Switch back to Azure Databricks. Select **Workspace** in the left-hand menu. Right-click within Workspace, then select **Create** --> **Folder**.
...
1.  Select **Monitor** in the left-hand menu. You will be able to see your pipeline activity in progress as well as the status of past runs.
...
1.  Run the cell. You should see a table displayed with the scored data. Scroll all the way to the right. There you will find the `prediction` column containing the flight delay prediction provided by your machine learning model.
